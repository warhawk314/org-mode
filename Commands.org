...
** Cygwin Notes
(16/2/18: note: currently Spacemacs based on NT Emacs (i.e. Windows Build) is used at Work)
...
Shared Connections (Putty)
Note on Windows best to set "use Shared Connections" in Putty setup for SSH (make sure saved in Session at top)
...
** Source Code Tagging (e.g. CTAGS, GNU Global) 
...
- Check that Exuberant ctags is being used (i.e. one in cygwin)
...
Followed layer instructions:
Installed Exuberant ctags 5.8.1 in cygwin (/usr/bin)
cp gtags.conf (from global download folder) to ~/.globalrc

Tried to install windows versions of global and tags. Trying to setup path in
.spacemacs to windows version, but emacs complains about symbol is void at
startup. Something wrong with path setting !
https://www.gnu.org/software/emacs/manual/html_node/elisp/Void-Variables.html
Try incremental changes in .spacemacs

Maybe try going back to GNU Global instructions.

[2018-02-15 Thu] CTAGS seems to work now at work ! See below.
...
Simple test at work seems to work ok:
  ...
  Define variable *projectile-tags-command* in DOT spacemacs, e.g.: 
  (setq projectile-tags-command "c:\\ctags\\ctags.exe -R -e")

  See https://simpletutorials.com/c/2826/How+to+setup+etags+for+fuzzy+symbol+searching+in+Spacemacs+on+Windows

  Script to use for TAGS file generation with ctags
  https://simpletutorials.com/c/2970/Python%20script%20for%20searching%20directories%20and%20outputting%20file%20paths
** Projectile Search and RIPGREP 
See also: https://gist.github.com/pesterhazy/fabd629fbb89a6cd3d3b92246ff29779

SPC / should allow you to search in the current project (set so that SPC / uses RIPGREP).

You can prefix patterns with options (RIPGREP).
Useful ones:

-g*.H a.*hack        - search for the Reg Exp a.*hack in all .H files, 
                       NOTE *NO* space in '-g*.H' !! (works well !)
-g*.{cpp,h} a.*hack  - reg ex search in all .cpp and .h files (works well !)
-C5 foo              - search for "foo" but show 5 lines of context before and after the match, 
                       NOTE the space in '-C5 foo' (works well !)
